...
void circles_set()
{
    ...
    cost_circles[0x50] = 2;   // I_MUL_0
    cost_circles[0x51] = 2;   // I_MUL_1
...
int main(int argc, char *argv[])
{
	...
	for(;;)
	{
		...
		switch(op)
		{
			...
			// 计算运行时间（以毫秒为单位）
    		double time_spent = (double)(end_time - start_time) / CLOCKS_PER_SEC * 1000.0;
			printf("程序运行时间: %.2f 毫秒\n", time_spent);
			printf("程序总消耗%d个周期数",total_circles);
			exit(0);
