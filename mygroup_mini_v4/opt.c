...
int build_CFG(TAC *tac_first){
    ...
    for(int i = 0; i < cur_headers - 1; i++){
        ...
        else if(BB_array[i]->out->op == TAC_IFZ || BB_array[i]->out->op == TAC_IF){
...
/*       
        ...
        //printf("\t simple opt done!\n");
        // //  // 常量传播+常量折叠优化
         opt_cnt += do_opt.const_opt(BB_array, BB_num, tac_first);
        //  printf("\t const_opt done!\n");
        //  print_cfg();

        strenth_reduce();
        // printf("\t  strenth_reduce done!\n"); 
        // print_alltac();
        BB_num = build_CFG(tac_first);
        bb_num=BB_num;
        // printf("bb_num:%d,round:%d\n",bb_num,round);
...
void print_alltac()
{
    ...
    {
        tac_print(cur);
        if(cur->prev)printf("  have prev--");
        if(cur->next)printf("  have next\n");
        printf("\n");
    }
}
